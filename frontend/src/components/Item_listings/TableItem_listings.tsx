...
import { loadColumns } from './configureItem_listingsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import CardItem_listings from './CardItem_listings';

const perPage = 10;

const TableSampleItem_listings = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: item_listingsNotify,
    refetch,
  } = useAppSelector((state) => state.item_listings);
  ...
  useEffect(() => {
    if (item_listingsNotify.showNotification) {
      notify(
        item_listingsNotify.typeNotification,
        item_listingsNotify.textNotification,
      );
    }
  }, [item_listingsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/item_listings/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/item_listings/item_listings-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {item_listings && Array.isArray(item_listings) && !showGrid && (
        <CardItem_listings
...
export default TableSampleItem_listings;
